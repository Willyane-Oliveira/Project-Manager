<template>
    <v-layout row wrap>
        <v-flex xs12>
            <h2 class="display-1 mb-2">Schedule</h2>
        </v-flex>
        ...
        <v-flex xs6>
            ...
            <v-btn dark @click="goToToday()">Go to today</v-btn>
        ...
        <v-flex xs6>
            <v-card color="green lighten-5" v-if="date">
                ...
                <v-card-text>
                    <p>Events total: {{ today.length }}</p>

                    <v-list>
                        <v-list-tile v-for="event, index in today" :key="index">
                            <v-list-tile-content>
                                <v-list-tile-title>{{ event.description }}</v-list-tile-title>
                                <v-list-tile-sub-title>{{ event.due_date.substr(10, event.due_date.length) }}</v-list-tile-sub-title>
                            </v-list-tile-content>
                        </v-list-tile>
                    </v-list>
                </v-card-text>
